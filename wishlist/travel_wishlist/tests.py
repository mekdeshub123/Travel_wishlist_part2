...
class TestImageUpload(TestCase):
    ...
    def setUp(self):
        user = User.objects.get(pk=1)
        self.client.force_login(user)
        self.MEDEA_ROOT = tempfile.mkdtemp()

    def tearDown(self):
        print('todo delete temp directory, temp image')

    def create_temp_image_file(self):
        handle, temp_img_file = tempfile.mkstemp(suffix='.jpg')
        img = Image.new('RGB', (10, 10))
        img.save(temp_img_file, format='JPEG')
        retern temp_img_file 

    def test_upload_new_image_for_own_place(self):
    img_file_path = self.create_temp_image_file()
    with self.settings(MEDIA_ROOT=self.MEDEA_ROOT):
    with open(img_file_path, 'rb')as img_file:
    res = self.client.post(reverse('place_details', kwargs={'place_pk': 1}), {'photo': img_file}, follow=True)
    ...
    #def test_c hange_image_for_own_place_expect_old_deleted(self):
